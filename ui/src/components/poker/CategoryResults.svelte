...
>
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
    {#each voteCategories as category}
      ...
      <div class="bg-gray-100 dark:bg-gray-700 rounded-lg p-4" data-testid={`result-${category.id}`}>
        ...
        <div class="mt-2 text-3xl font-bold" data-testid="category-average">{group?.average || '待评点'}</div>
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-300">平均点数 · {group?.count ?? 0} 个有效评分</p>
        {#if votes.length}
  ...
  >
    ...
    <output class="text-3xl font-bold text-green-700 dark:text-lime-400" data-testid="category-total"
      >{estimation?.total || '待评点'}</output
    >
  </div>
  <p class="mt-3 text-sm text-gray-600 dark:text-gray-300">
    {#if !estimation?.total}每类至少需要一个数字评分；没有工作量请投 0。{/if}
    未投票和“？”不计入平均值。合计使用未取整的均值，结果保留两位小数。
  </p>
</section>
