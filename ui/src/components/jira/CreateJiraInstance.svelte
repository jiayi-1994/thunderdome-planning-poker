<script lang="ts">
  ...
  function handleSubmit(event: Event) {
    ...
    xfetch(`/api/users/${$user.id}/jira-instances`, { body, signal: requestController.signal })
      .then((res: Response) => res.json())
      .then(function () {
        notifications.success('Jira connection verified and saved');
        handleCreate();
        toggleClose();
      })
  ...
  <form onsubmit={handleSubmit} name="createjirainstance">
    <fieldset disabled={submitting}>
      ...
      <div class="mb-4">
        ...
        />
        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
          Creating this connection automatically checks your Jira credentials before saving.
        </p>
      </div>
      <div class="text-right">
        <div>
          <SolidButton type="submit" disabled={submitting}>
            {submitting ? 'Checking and saving...' : $LL.create()}
          </SolidButton>
        </div>
      </div>
    </fieldset>
